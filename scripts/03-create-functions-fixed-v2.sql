...
BEGIN
  FOR alert_record IN 
    SELECT id, user_id, alert_term, alert_type, is_active FROM public.alerts 
    ...
    AND (
      ...
      (alert_type = 'phone' AND LOWER(NEW.phone_number) LIKE '%' || LOWER(alert_term) || '%') -- Updated to check phone_number
    )
  LOOP
