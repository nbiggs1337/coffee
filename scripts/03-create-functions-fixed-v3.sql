...
-- We are updating it to include the new post's ID in the notification.
...
BEGIN
    ...
    LOOP
        -- Check for a match based on the alert type
        IF (alert.alert_type = 'name' AND NEW.name ILIKE '%' || alert.alert_term || '%') OR
